<template>
  <div class="pt-16 px-8 pb-8">
    ...
    <div
      v-if="filteredWomenPerfumes.length"
      class="grid grid-cols-1 gap-10 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4"
    >
      <div v-for="item in filteredWomenPerfumes" :key="item.id" class="shadow-lg px-3 py-2">
        <div>
          <img
            :src="item.image"
            alt="Women Perfume"
...
const fetchWomenPerfumes = async () => {
  try {
    ...
    womenPerfumes.value = perfumes
  } catch (error) {
    console.error('Error fetching women perfumes:', error)
  }
}

onMounted(fetchWomenPerfumes)
...
const filteredWomenPerfumes = computed(() => {
  return womenPerfumes.value
    .filter((perfume) => perfume.type === 'women')
    .sort((a, b) => {
...
const editPrice = (id) => {
  const perfume = womenPerfumes.value.find((item) => item.id === id)
  if (perfume) {
